 ...
 * Uses mapped alternates when available; otherwise falls back to locale home.
 */
export function LanguageSwitcher({ currentPath, className }: LanguageSwitcherProps) {
  ...
  return (
    <nav aria-label="Language" className={cn("flex items-center gap-1", className)}>
      <Link href={esHref} className={linkClass(!isEnglish)} lang="es-MX">
        Español
      </Link>
      <span aria-hidden="true" className="text-xs text-muted-foreground/60">
        |
      </span>
      <Link href={enHref} className={linkClass(isEnglish)} lang="en">
        English
      </Link>
    </nav>
  );
