...
const categories = [
  ...
  },
  {
    ...
    description: "Registro por email y pagos desde wallets",
    icon: "shield" as const,
  },
  {
    label: "Fiat México",
    href: "/casinos-fiat",
    description: "OXXO, SPEI, tarjetas y métodos locales",
    icon: "fiat" as const,
  },
  {
    label: "Bonos",
    href: "/bonos",
    description: "Promociones, rollover, límites y condiciones",
    icon: "bonus" as const,
  },
] as const;
...
const secondaryLinks = [
  ...
  { label: "USDT", href: "/guias/casinos-con-usdt-mexico" },
...
/**
 * Compact homepage category navigation — single navigation system for V3.
 */
export function QuickCategories({ className }: { className?: string }) {
  return (
    <section aria-labelledby="explora-categoria-heading" className={cn("py-6 sm:py-8", className)}>
      <HomepageContainer>
        <div className="max-w-2xl space-y-1">
          <h2
            id="explora-categoria-heading"
            className="text-lg font-bold tracking-tight text-foreground sm:text-xl"
          >
            Explora por categoría
          </h2>
          <p className="text-sm text-muted-foreground">
            Encuentra la sección adecuada sin recorrer toda la página.
          </p>
        </div>
