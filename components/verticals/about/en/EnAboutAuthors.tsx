...
import {
  EN_ABOUT_AUTHOR_DISPLAY,
  EN_ABOUT_AUTHOR_ORDER,
...
function orderEnAboutAuthors(authors: Author[]): Author[] {
  return [...authors].sort((a, b) => {
    const ai = EN_ABOUT_AUTHOR_ORDER.indexOf(
      a.id as (typeof EN_ABOUT_AUTHOR_ORDER)[number],
    );
    const bi = EN_ABOUT_AUTHOR_ORDER.indexOf(
      b.id as (typeof EN_ABOUT_AUTHOR_ORDER)[number],
    );
    const aRank = ai === -1 ? EN_ABOUT_AUTHOR_ORDER.length : ai;
    const bRank = bi === -1 ? EN_ABOUT_AUTHOR_ORDER.length : bi;
    return aRank - bRank;
  });
}
...
export function EnAboutAuthors() {
  const authors = orderEnAboutAuthors(getAuthors()).filter(
    (author) =>
  ...
  return (
    <section
      id="about-authors"
      aria-labelledby="en-about-authors-heading"
    ...
    >
      <div className="max-w-3xl space-y-1">
        <h2
          id="en-about-authors-heading"
          className="text-xl font-bold tracking-tight text-foreground sm:text-2xl lg:text-[1.65rem]"
        >
          Authors and editorial team
        </h2>
        <p className="text-sm text-muted-foreground sm:text-base">
          Clear attribution. LinkedIn is used for identity and distribution, not as a factual source
          for operators or ratings.
        </p>
      </div>
      ...
      <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
        {authors.map((author) => {
          const display =
            EN_ABOUT_AUTHOR_DISPLAY[
              author.id as keyof typeof EN_ABOUT_AUTHOR_DISPLAY
            ];
