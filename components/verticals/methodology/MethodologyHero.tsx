...
import {
  METHOD_FOCUS_PILLS,
  METHOD_HERO_PANEL_ITEMS,
} from "@/components/verticals/methodology/methodology-config";
    ...
    <header className="relative mb-7 overflow-hidden rounded-2xl border border-white/12 bg-gradient-to-br from-[#10182a] via-[#0A1931] to-[#0c1424] shadow-[0_0_40px_-24px_rgba(255,184,0,0.3)] sm:mb-10">
      ...
      <div className="relative grid gap-4 p-5 sm:p-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-center lg:gap-8 lg:p-8">
        <div className="space-y-3">
          ...
          <ul className="flex flex-wrap gap-2" aria-label="Enfoque metodológico">
            {METHOD_FOCUS_PILLS.map((badge) => (
              <li
                key={badge}
        ...
        <aside
          aria-labelledby="method-hero-panel-heading"
        ...
        >
          <h2
            id="method-hero-panel-heading"
            className="text-sm font-semibold tracking-tight text-foreground"
          >
            Qué analizamos
          </h2>
          <ul className="mt-2 grid grid-cols-2 gap-1.5">
            {METHOD_HERO_PANEL_ITEMS.map((title) => (
              <li
                key={title}
          ...
          <p className="mt-2 text-[0.65rem] leading-snug text-muted-foreground">
            Términos, productos y disponibilidad del operador pueden cambiar. Confírmalos en el
            sitio oficial.
          </p>
        </aside>
      </div>
