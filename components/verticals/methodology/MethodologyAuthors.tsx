...
import { METHOD_AUTHOR_ORDER } from "@/components/verticals/methodology/methodology-config";
...
function orderMethodAuthors(authors: Author[]): Author[] {
  return [...authors].sort((a, b) => {
    const ai = METHOD_AUTHOR_ORDER.indexOf(
      a.id as (typeof METHOD_AUTHOR_ORDER)[number],
    );
    const bi = METHOD_AUTHOR_ORDER.indexOf(
      b.id as (typeof METHOD_AUTHOR_ORDER)[number],
    );
    const aRank = ai === -1 ? METHOD_AUTHOR_ORDER.length : ai;
    const bRank = bi === -1 ? METHOD_AUTHOR_ORDER.length : bi;
    return aRank - bRank;
  });
}
...
export function MethodologyAuthors() {
  const authors = orderMethodAuthors(getAuthors()).filter(
    (author) =>
  ...
  return (
    <section
      id="autores"
      aria-labelledby="method-authors-heading"
    ...
    >
      <div className="max-w-3xl space-y-1">
        <h2
          id="method-authors-heading"
          className="text-xl font-bold tracking-tight text-foreground sm:text-2xl lg:text-[1.65rem]"
        >
          Autores y equipo editorial
        </h2>
        <p className="text-sm text-muted-foreground sm:text-base">
          Las reseñas llevan atribución clara. LinkedIn se usa para identidad y distribución, no
          como fuente factual de operadores o calificaciones.
        </p>
      </div>
      ...
      <ul className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
        {authors.map((author) => {
          ...
          const role = isOrg ? "Equipo editorial" : "Fundador y editor";
