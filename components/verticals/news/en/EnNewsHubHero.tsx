...
const FOCUS_BADGES = [
  ...
  "Sports betting",
  "Analysis and opinion",
  "Global and Mexico-facing",
] as const;

/**
 * Compact two-column English News Hub hero — editorial, no affiliate CTA.
 */
export function EnNewsHubHero() {
  return (
      ...
      <div className="relative grid gap-4 p-5 sm:p-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-center lg:gap-8 lg:p-8">
        <div className="space-y-3">
          <span className="inline-flex items-center rounded-full border border-sky-400/40 bg-sky-500/10 px-3 py-1 text-xs font-medium text-sky-200">
            Global · News · Analysis · Opinion
          </span>
          <h1 className="text-[1.75rem] font-bold leading-tight tracking-tight text-foreground sm:text-[2.1rem] lg:text-[2.3rem]">
            Casino, Crypto & Betting News and Analysis
          </h1>
          <p className="max-w-2xl text-base leading-relaxed text-muted-foreground">
            Editorial updates, analysis and author commentary on regulation, operators, bonuses,
            payments and sports betting. We separate reported facts from opinion and cite sources
            when they apply.
          </p>
          <ul className="flex flex-wrap gap-2" aria-label="News hub focus">
        ...
        <aside
          aria-labelledby="en-news-hero-cover-heading"
        ...
        >
          ...
          <p className="mt-2 text-[0.65rem] leading-snug text-muted-foreground">
            We do not fabricate events, statistics or urgency. 18+ only. Gamble responsibly.
          </p>
        </aside>
      </div>
    </header>
