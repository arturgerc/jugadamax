...
function EnNewsArticleAuthorCard({ author }: { author: Author }) {
  ...
  const role = authorRoleDisplay(author);
  const bio =
    author.id === "redaccion-jugadamax"
      ? "The JugadaMax editorial team evaluates operators with a published methodology, prioritising clarity, payments, licensing information and user experience."
      : author.id === "arturs-stoliks"
        ? "Arturs Stoliks coordinates JugadaMax as an independent editorial project on online casinos, sports betting and digital payments. The focus is clear guides, cautious reviews and responsible content for adults 18+."
        : author.bio;

  return (
    <aside
      aria-labelledby="en-article-author-heading"
    ...
    >
      <p className="text-[0.7rem] font-semibold uppercase tracking-[0.14em] text-accent">
        {isOrg ? "Editorial team" : "Author"}
      </p>
      <h2
        id="en-article-author-heading"
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        >
          {isOrg ? "About the team" : "View profile"}
        </Link>
        {linkedInHref ? (
          <a
            href={linkedInHref}
      ...
      {linkedInHref ? (
        <p className="mt-3 text-[0.7rem] leading-snug text-muted-foreground">
          LinkedIn is used for identity and distribution, not as a factual source for the article.
        </p>
      ) : null}
    </aside>
  );
...
/**
 * Individual English news article template — fact/opinion separation, sources, related.
 */
export function EnNewsArticleTemplate({
  card,
  related,
}: {
  card: EnResolvedNewsCard;
  related: EnResolvedNewsCard[];
}) {
  ...
  return (
    <article className="mx-auto w-full max-w-3xl space-y-6 sm:space-y-8">
          ...
          <p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-muted-foreground">
            ...
            <span>
              ...
              <time dateTime={card.displayDateIso}>
                {formatEnNewsDate(card.displayDateIso)}
              </time>
            </span>
          </p>
        </div>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <p className="rounded-lg border border-emerald-500/20 bg-emerald-500/8 px-3 py-2 text-xs leading-relaxed text-emerald-100/90">
            <span className="font-semibold text-emerald-200">
              {article.factLabel ?? "Facts"}:
            </span>{" "}
            the body summarises information attributed to the sources cited below.
          </p>
          <p className="rounded-lg border border-violet-500/20 bg-violet-500/8 px-3 py-2 text-xs leading-relaxed text-violet-100/90">
            <span className="font-semibold text-violet-200">
              {article.opinionLabel ?? "Opinion"}:
            </span>{" "}
            the author commentary is editorial judgement, not an additional fact.
          </p>
        </div>
      ) : null}
              ...
              <li key={source.url} className="text-sm">
                ...
                {source.publisher ? (
                  <span className="mt-0.5 block text-xs text-muted-foreground">
                    {source.publisher}
                    {source.publishedAt
                      ? ` · ${formatEnNewsDate(source.publishedAt)}`
                      : ""}
                  </span>
                ) : null}
                {source.note ? (
      ...
      <EnNewsArticleAuthorCard author={author} />

      {article.relatedLinks && article.relatedLinks.length > 0 ? (
        <nav aria-label="Related article links">
      ...
      {related.length > 0 ? (
        <section aria-labelledby="en-more-news-heading">
          <h2
            id="en-more-news-heading"
            className="text-base font-bold text-foreground sm:text-lg"
          >
            More in news
          </h2>
      ...
      <p className="border-t border-border/60 pt-6 text-xs text-muted-foreground">
        Adults 18+ only. Gamble responsibly.{" "}
        <Link
