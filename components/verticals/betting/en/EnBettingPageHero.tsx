...
import { EN_BETTING_QUICK_LINKS } from "@/components/verticals/betting/en/en-betting-page-config";
...
/**
 * Compact two-column betting hero for /en/betting.
 * Mobile: tighter spacing; hero anchor pills hidden (quick-nav covers them).
 */
export function EnBettingPageHero() {
  return (
      ...
      <div className="relative grid gap-3 p-4 sm:gap-4 sm:p-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:items-center lg:gap-8 lg:p-8">
        <div className="space-y-2 sm:space-y-3">
          <span className="inline-flex items-center rounded-full border border-accent/40 bg-accent/10 px-2.5 py-0.5 text-[0.7rem] font-medium text-accent sm:px-3 sm:py-1 sm:text-xs">
            Global · Sports betting · Editorial
          </span>
          <h1 className="text-[1.65rem] font-bold leading-tight tracking-tight text-foreground sm:text-[2.15rem] lg:text-[2.35rem]">
            Sports Betting Sites — Global & Mexico-Facing Reviews
          </h1>
          <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground sm:text-base">
            Compare sportsbooks on markets, payments, sports promotions, KYC, withdrawals and
            editorial assessment. JugadaMax remains casino-first; this section covers sportsbook as
            an additional vertical.
          </p>
          <ul className="flex flex-wrap gap-1.5 sm:gap-2" aria-label="Page focus">
        ...
        <aside
          aria-labelledby="en-betting-hero-check-heading"
          className="rounded-xl border border-white/10 bg-[#111417]/80 p-2.5 sm:p-4"
        >
          <h2
            id="en-betting-hero-check-heading"
          ...
          <p className="mt-1.5 text-[0.65rem] leading-snug text-muted-foreground sm:mt-2">
            Odds, markets and promotions change. Always confirm on the operator.{" "}
            <Link
          ...
          >
            {EN_BETTING_QUICK_LINKS.slice(0, 3).map((link) => (
              <TrackedLink
                key={link.href}
                href={link.href}
