...
/**
 * Responsive comparison of all six active EN betting operators.
 * Mobile: closed-by-default details. Desktop: table.
 * No visible rank numbers — order is editorial only.
 */
export function EnBettingComparison() {
  const rows = resolveEnBettingCards();
  if (rows.length === 0) return null;

  return (
    <section
      id="betting-comparison"
      aria-labelledby="en-betting-comparison-heading"
      className="mb-5 scroll-mt-24 sm:mb-10 lg:mb-12"
    >
      <div className="max-w-3xl space-y-1">
        <h2
          id="en-betting-comparison-heading"
          className="text-xl font-bold tracking-tight text-foreground sm:text-2xl"
        >
          Sportsbook comparison
        </h2>
        <p className="text-sm text-muted-foreground sm:text-base">
          Summary of active operators. Methods, promotions, verification and withdrawals depend on
          each operator, account and location. Order is editorial and may differ from individual
          scores.
        </p>
      </div>
      ...
      <details className="group mt-3 rounded-xl border border-white/10 bg-[#111417]/60 open:border-primary/20 sm:mt-4 md:hidden">
        ...
        >
          <span className="truncate">View active operator comparison</span>
          <span
            aria-hidden="true"
