...
import { resolveEnComparisonRows } from "@/components/verticals/bonuses/en/en-bonus-data";
import {
  BONUS_SURFACES,
 ...
 */
export function EnBonusComparison() {
  const rows = resolveEnComparisonRows();
  if (rows.length === 0) return null;

  return (
    <section
      id="bonus-comparison"
      aria-labelledby="bonus-comparison-heading"
      className="mb-7 scroll-mt-24 sm:mb-10 lg:mb-12"
    >
      <div className="max-w-3xl space-y-1">
        <h2 id="bonus-comparison-heading" className={sectionHeadingClassName()}>
          Quick promotions comparison
        </h2>
        <p className="text-sm text-muted-foreground sm:text-base">
          Summary of published offers. Minimum deposit, wagering, validity and codes depend on each
          operator, account and location. “Not published” means JugadaMax does not have that value
          confirmed in the editorial record.
        </p>
      </div>
      ...
      <details className="group mt-4 rounded-xl border border-cyan-500/15 bg-[#111417]/70 open:border-cyan-400/25 md:hidden">
        ...
        >
          <span className="flex min-w-0 items-center gap-2">
            <span className="truncate">View promotions comparison</span>
            ...
            <tbody>
              {rows.map((row, index) => (
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    {row.promoCode ? (
                      <span className={BONUS_SURFACES.promoChip}>{row.promoCode}</span>
                    ) : (
                  ...
                  <td className="px-3 py-3">
                    <div className="flex w-[9.5rem] flex-col gap-1">
                      ...
                      >
                        <span className="line-clamp-2">{row.ctaLabel}</span>
                      </TrackedLink>
                      {row.reviewHref ? (
                        <TrackedLink
                          href={row.reviewHref}
